...
class UpdateVideosFrame(VideoFrame):
    ...
    def _create_widgets(self):
        ...
        ).grid(row=3, column=1, sticky=(N, W, E, S))

        combobox = ttk.Combobox(self, textvariable=self._update_column)
        combobox['state'] = 'readonly'
        combobox['values'] = ('rating')
        combobox.set('rating') 
        combobox.grid(column=1, row=5, sticky=(W, E, N))
        
        ttk.Entry(
            self,
            textvariable=self._new_val
        ).grid(row=7, column=1, sticky=(N, W, E, S))
        ...
        self.__update_btn.grid(row=8, column=1, sticky=(N, W, E, S))
